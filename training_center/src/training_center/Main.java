...
public class Main {
	...
	public static void main(String[] args) {
		...
		try {
			...
			Collection<Lesson> lessons = Training_center.getInstance().getLessonDAO().getNextLessonsFor((long)3, std);
			Iterator<Lesson> iter = lessons.iterator();
			while (iter.hasNext()) {
				Lesson s = iter.next();
				System.out.println(s.getTime());
			}
			Training_center.getInstance().getStudentDAO().deleteStudent(std, true);
		} catch (Exception e) {
			System.out.print(e.getMessage());
		}
	}
